...
public abstract class MCTNode {
	...
	public MCTNode(ACTIONS action, MCTNode parent) {
		...
		case ACTION_DOWN:
			if (!orientation.equals(YoloKnowledge.ORIENTATION_NULL)
					&& !orientation.equals(YoloKnowledge.ORIENTATION_DOWN))
				noMove = true;
			y++;
			break;
		case ACTION_UP:
			if (!orientation.equals(YoloKnowledge.ORIENTATION_NULL)
					&& !orientation.equals(YoloKnowledge.ORIENTATION_UP))
				noMove = true;
			y--;
			break;
		case ACTION_RIGHT:
			if (!orientation.equals(YoloKnowledge.ORIENTATION_NULL)
					&& !orientation.equals(YoloKnowledge.ORIENTATION_RIGHT))
				noMove = true;
			x++;
			break;
		case ACTION_LEFT:
			if (!orientation.equals(YoloKnowledge.ORIENTATION_NULL)
					&& !orientation.equals(YoloKnowledge.ORIENTATION_LEFT))
				noMove = true;
			x--;
			break;
		default:
			// TODO: Action use auf singleton checken! Wenn schon geschossen,
			// dann true!
			noMove = true;
		}
		...
		if (MCTHandler.heuristicToUse != null) {
			averageRewards[MCTHandler.heuristics
					.getIndexOfHeuristic(MCTHandler.heuristicToUse.GetType())] = MCTHandler.heuristicToUse
							.getModdedHeuristic(parent.getState(), x, y, orientation);
		}
	...
	public YoloState simulate(boolean useScoreLookahead, boolean forceNoRandom) {
		...
		if (YoloKnowledge.instance.positionAufSpielfeld(newState.getAvatarX(), newState.getAvatarY())) {
			if (MCTHandler.heuristicToUse != null
					&& MCTHandler.heuristicToUse.EvaluateWithoutNormalisation(newState) == 0) {
				// Already at target!
				targetReached = true;
			}
		}
		...
		while (!oldState.isGameOver() && currentDepth < MCTHandler.ROLLOUT_DEPTH) {
			...
			// shouldBackTrack |=
			// !YoloKnowledge.instance.haveEverGotScoreWithoutWinning() &&
			// rootScore > newState.getGameScore();
			if (shouldBackTrack) {
				...
				if (!targetReached && MCTHandler.heuristicToUse != null
						&& YoloKnowledge.instance.positionAufSpielfeld(newState.getAvatarX(), newState.getAvatarY())) {
	...
	public MCTNode uSBSelectChild() {
		...
		for (int i = 0; i < hvVals.length; i++) {
			...
			double uctValue = hvVals[i]
					+ Math.sqrt(2) * Math.sqrt(Math.log(this.visits + 1) / (child.visits + MCTHandler.epsilon));

			// small sampleRandom numbers: break ties in unexpanded nodes
			uctValue = Utils.noise(uctValue, MCTHandler.epsilon, MCTHandler.rnd.nextDouble()); // break
																								// ties
																								// randomly
	...
	public ArrayList<ACTIONS> getValidActions(YoloState curState) {
		...
		Observation stochasticKiller = YoloKnowledge.instance.getPossibleStochasticKillerAt(curState,
				curState.getAvatarX(), curState.getAvatarY());
		boolean shouldMove = stochasticKiller != null;
		boolean canUse = shouldMove && validActions.contains(ACTIONS.ACTION_USE)
				&& YoloKnowledge.instance.canInteractWithUse(curState.getAvatar().itype, stochasticKiller.itype)
	...
	private boolean observationIsInFrontOfAvatar(YoloState curState, Observation observation) {
	...
	private boolean couldGetKilledByEnemyIfIUseAction(YoloState curState, ACTIONS action, boolean shouldMove,
			boolean canUse) {
		if (shouldMove) {
